...
public interface IgseMapper extends Mapper<Igse> {
    ...
    @Select("select * from (select *,row_number() over(PARTITION by customer_id order by submission_date desc) as rowid\n" +
            ...
            "              ) a where rowid<=2")
    List<Reading> findTop2Record(@Param("propertyType") String propertyType, @Param("roomNum") String roomNum);
}
